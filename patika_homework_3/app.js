...
// let categorySet = new Set(menu.map(item => item.category));
// categorySet.add("All");
// categorySet.forEach((item, index) => {
//   let btnAll = document.createElement('button');
//   btnAll.innerHTML = item;
//   btnAll.setAttribute("class", "btn btn-outline-dark btn-item");
//   btnAll.setAttribute('id', index);
//   btnAll.setAttribute('onClick',item)

//   buttonContainerDOM.append(btnAll);
// });
...
//   });
//   menuItemsDOM.innerHTML = allFoods;
// }
...
const categoryList = () => {
  const categoryBtns = categories
    .map((category,index) => { 
      return `<button class="btn btn-outline-dark btn-item ${index == 0 ? 'active' : ''}" data-id=${category}>${category}</button>`;
    })
    .join("");
